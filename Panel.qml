...
Panel {
  ...
  property string internetRouteMode: "Auto"
  property string pendingRouteMode: ""
  property string routeError: ""
  property string bothPolicy: "Auto"
  property string pendingBothPolicy: ""
  property var routeProcesses: []
  property string processSearch: ""
  property bool processAdvanced: false
  property real processListScrollY: 0
  property bool restoringProcessListScroll: false
  readonly property var routeModes: ["Auto", "Ethernet", "Wi-Fi", "Both"]
  ...
  function selectRouteByDelta(delta) {
    routeIndex = Math.max(0, Math.min(routeModes.length - 1, routeIndex + delta))
  }

  function activateRoute() {
    if (routeIndex < 0 || routeIndex >= routeModes.length) return
    setInternetRoute(routeModes[routeIndex])
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!routeProc.running) {
      routeProc.command = ["omarchy-internet-route"]
  ...
  function updateInternetRoute(raw) {
    var value = String(raw || "").trim()
    internetRouteMode = value || "Auto"
    if (internetRouteMode === "Both" && !bothPolicyProc.running) bothPolicyProc.running = true
  }

  function routeCommand(mode) {
    var command = "omarchy-internet-route"
    if (mode) command += " " + Util.shellQuote(mode)
    return command
  }

  function setInternetRoute(mode) {
    if (!root.bar || !mode || actionProc.running) return
    routeError = ""
    root.pendingRouteMode = mode
    actionProc.command = ["bash", "-c", root.routeCommand(mode)]
    actionProc.running = true
  }

  function updateRouteProcesses(raw) {
    var lines = String(raw || "").split(/\r?\n/)
    var next = []
    for (var i = 0; i < lines.length; i++) {
      var fields = lines[i].split("\t")
      if (fields.length < 2 || !/^\d+$/.test(fields[0])) continue
      next.push({ pid: fields[0], name: fields[1], route: fields[2] || "", gui: fields[3] === "1" })
    }
    // `ps` output order can change between polls. Keep the model stable so
    // routine refreshes don't make the process list jump back to the top.
    next.sort(function(a, b) {
      var byName = a.name.localeCompare(b.name)
      return byName !== 0 ? byName : Number(a.pid) - Number(b.pid)
    })
    var unchanged = next.length === routeProcesses.length
    for (var j = 0; unchanged && j < next.length; j++) {
      var oldProcess = routeProcesses[j]
      var newProcess = next[j]
      unchanged = oldProcess.pid === newProcess.pid
        && oldProcess.name === newProcess.name
        && oldProcess.route === newProcess.route
        && oldProcess.gui === newProcess.gui
    }
    if (unchanged) return
    restoringProcessListScroll = true
    routeProcesses = next
    Qt.callLater(function() {
      processListView.contentY = Math.max(0, Math.min(processListScrollY,
        processListView.contentHeight - processListView.height))
      restoringProcessListScroll = false
    })
  }

  function processesFor(route) {
    var next = []
    var groups = groupedRouteProcesses()
    for (var i = 0; i < groups.length; i++) {
      if (groups[i].route === route && (processAdvanced || groups[i].isApp)) next.push(groups[i])
    }
    return next
  }

  function groupedRouteProcesses() {
    var groups = Object.create(null)
    var ordered = []
    for (var i = 0; i < routeProcesses.length; i++) {
      var process = routeProcesses[i]
      var group = groups[process.name]
      if (!group) {
        group = { name: process.name, pids: [], routes: [], isApp: false }
        groups[process.name] = group
        ordered.push(group)
      }
      group.pids.push(process.pid)
      if (process.gui) group.isApp = true
      if (group.routes.indexOf(process.route) < 0) group.routes.push(process.route)
    }
    for (var j = 0; j < ordered.length; j++) {
      var routes = ordered[j].routes
      ordered[j].route = routes.length === 1 ? routes[0] : routes.length > 1 ? "Mixed" : ""
    }
    return ordered
  }

  function filteredRouteProcesses() {
    var query = processSearch.trim().toLowerCase()
    var next = []
    var groups = groupedRouteProcesses()
    var appNames = Object.create(null)
    for (var g = 0; g < groups.length; g++) {
      if (groups[g].isApp) appNames[groups[g].name] = true
    }
    var source = []
    if (processAdvanced) {
      for (var p = 0; p < routeProcesses.length; p++) {
        var item = routeProcesses[p]
        source.push({ pid: item.pid, name: item.name, route: item.route,
          isApp: !!appNames[item.name], pids: [item.pid] })
      }
      ...
      source = groups.filter(function(group) { return group.isApp })
    }
    for (var i = 0; i < source.length; i++) {
      var process = source[i]
      if (query !== "") {
        var matchesName = process.name.toLowerCase().indexOf(query) >= 0
        var matchesPid = processAdvanced && process.pid.indexOf(query) >= 0
        if (!matchesName && !matchesPid) continue
      }
      var previous = next.length > 0 ? next[next.length - 1] : null
      // Mark the two sections in Advanced mode: windowed apps first, then all
      // remaining user-owned processes.
      if (!processAdvanced || !previous || previous.isApp !== process.isApp) {
        process.showSectionHeader = true
        process.sectionTitle = processAdvanced
          ? (process.isApp ? "RUNNING APPS" : "OTHER PROCESSES")
          : "RUNNING APPS"
      }
      next.push(process)
    }
  ...
  function moveProcesses(pids, route) {
    if (actionProc.running) return
    var list = Array.isArray(pids) ? pids : [pids]
    actionProc.command = ["omarchy-internet-route", "process-route", list.join(","), route]
    actionProc.running = true
  }

  function setBothPolicy(policy) {
    if (actionProc.running || !policy) return
    if (policy === "Auto") {
      pendingBothPolicy = policy
      actionProc.command = ["bash", "-c", "omarchy-internet-route process-reset && omarchy-internet-route both-policy Auto"]
      actionProc.running = true
    } else {
      pendingBothPolicy = policy
      actionProc.command = ["omarchy-internet-route", "both-policy", policy]
      actionProc.running = true
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateInternetRoute(text)
    }
  }

  Process {
    id: routedProcessesProc
    command: ["omarchy-internet-route", "processes"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: root.updateRouteProcesses(text)
    }
  }

  Process {
    id: bothPolicyProc
    command: ["omarchy-internet-route", "both-policy"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        root.bothPolicy = String(text || "Auto").trim() === "Manual" ? "Manual" : "Auto"
        if (root.bothPolicy === "Manual" && !routedProcessesProc.running) routedProcessesProc.running = true
      }
    }
  }
  ...
  Timer {
    interval: 3000
    repeat: true
    running: root.opened && root.internetRouteMode === "Both" && root.bothPolicy === "Manual"
    onTriggered: if (!routedProcessesProc.running) routedProcessesProc.running = true
  }
  ...
  // Action runner for DNS provider and route changes. Wi-Fi actions use the
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (root.pendingRouteMode !== "") {
        if (exitCode === 0) root.internetRouteMode = root.pendingRouteMode
        else root.routeError = String(actionStderr.text || "Route change failed.").trim()
        root.pendingRouteMode = ""
        root.refresh()
      }
      if (root.pendingBothPolicy !== "") {
        if (exitCode === 0) root.bothPolicy = root.pendingBothPolicy
        root.pendingBothPolicy = ""
        if (!routedProcessesProc.running) routedProcessesProc.running = true
      }
      if (root.pendingBand !== "") {
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      onMoveRequested: function(dx, dy) {
        ...
        if (dy !== 0) {
          ...
          } else if (root.focusSection === "route") {
            if (dy < 0) {
              if (root.canSelectBand) {
                root.focusSection = "band"
        ...
        if (dx !== 0) {
          ...
          else if (root.focusSection === "route") root.selectRouteByDelta(dx)
      ...
      onActivateRequested: {
        if (root.cursorActive) {
          ...
          else if (root.focusSection === "route") root.activateRoute()
    ...
    Column {
      ...
      // Internet route / default gateway selection.
      PanelSeparator {
        foreground: root.bar.foreground
      }
      ...
      Column {
        ...
        Row {
          ...
          RouteModePill {
            mode: "Auto"
            index: 0
            tooltipText: "Auto: Ethernet priority with Wi-Fi fallback"
            width: routeRow.cellWidth
            onClicked: root.setInternetRoute(mode)
          }

          RouteModePill {
            mode: "Ethernet"
            index: 1
            tooltipText: "Internet via Ethernet (Wi-Fi as LAN only)"
            width: routeRow.cellWidth
            onClicked: root.setInternetRoute(mode)
          }
          ...
          RouteModePill {
            mode: "Wi-Fi"
            index: 2
            tooltipText: "Internet via Wi-Fi (Ethernet as LAN only)"
            width: routeRow.cellWidth
            onClicked: root.setInternetRoute(mode)
          }
          ...
          RouteModePill {
            mode: "Both"
            index: 3
            tooltipText: "Distribute new connections across Ethernet and Wi-Fi"
            width: routeRow.cellWidth
            onClicked: root.setInternetRoute(mode)
          }
        }
        ...
        Row {
          visible: root.internetRouteMode === "Both"
          spacing: Style.space(6)
          Button {
            text: "Auto"
            ...
            bordered: true
            active: root.bothPolicy === "Auto"
            onClicked: root.setBothPolicy("Auto")
          }
          Button {
            text: "Manual"
            fontSize: Style.font.bodySmall
            ...
            verticalPadding: Style.space(3)
            bordered: true
            active: root.bothPolicy === "Manual"
            onClicked: {
              root.setBothPolicy("Manual")
            }
          }
        }

        Column {
          visible: root.internetRouteMode === "Both" && root.bothPolicy === "Manual"
          width: parent.width
          spacing: Style.space(6)

          InfoLabel {
            visible: root.processAdvanced
            width: parent.width
            wrapMode: Text.WordWrap
            text: "Choose a PID to route that process instance. Existing connections stay on their current link."
          }
          ...
          Row {
            ...
            Repeater {
              ...
              delegate: Column {
                ...
                readonly property string routeName: modelData
                width: (parent.width - Style.space(8)) / 2
                spacing: Style.space(4)
                PanelSectionHeader {
                  text: modelData.toUpperCase() + " · " + root.processesFor(modelData).length
                  foreground: root.bar.foreground
                  fontFamily: root.bar.fontFamily
                }
                ListView {
                  ...
                  model: root.processesFor(routeColumn.routeName)
                  delegate: Button {
                    required property var modelData
                    property string destination: routeColumn.routeName === "Ethernet" ? "Wi-Fi" : "Ethernet"
                    width: parent.width
                    text: (routeColumn.routeName === "Ethernet" ? "→ " : "← ")
                      + modelData.name
                      + (modelData.pids.length > 1 ? " · " + modelData.pids.length : "")
                    fontSize: Style.font.bodySmall
                    foreground: root.bar.foreground
                    fontFamily: root.bar.fontFamily
                    ...
                    verticalPadding: Style.space(3)
                    bordered: true
                    onClicked: root.moveProcesses(modelData.pids, destination)
                  }
          ...
          Row {
            ...
            spacing: Style.space(6)
            PanelSectionHeader {
            text: root.processAdvanced ? "PROCESS INSTANCES" : "RUNNING APPS"
              foreground: root.bar.foreground
              fontFamily: root.bar.fontFamily
              width: parent.width - advancedPidButton.implicitWidth - parent.spacing
            }
            Button {
              id: advancedPidButton
              text: root.processAdvanced ? "Apps" : "Advanced · PID"
          ...
          TextField {
            width: parent.width
            placeholderText: root.processAdvanced ? "Filter by name or PID" : "Filter by app/process name"
            font.family: root.bar.fontFamily
          ...
          InfoLabel {
            visible: root.processSearch.trim() === "" && root.processAdvanced
            width: parent.width
            text: "Open apps are listed first; other processes follow below."
          }
          ...
          ListView {
            ...
            clip: true
            interactive: contentHeight > height
            onContentYChanged: if (!root.restoringProcessListScroll) root.processListScrollY = contentY
            model: root.filteredRouteProcesses()
            delegate: Item {
              required property var modelData
              required property int index
              width: ListView.view.width
              readonly property real headerHeight: root.processAdvanced && modelData.showSectionHeader ? Style.space(22) : 0
              height: Style.space(30) + headerHeight
              InfoValue {
                visible: root.processAdvanced && modelData.showSectionHeader
                x: Style.space(4)
                y: 0
              ...
              Row {
                ...
                InfoValue {
                  ...
                  text: root.processAdvanced
                    ? modelData.name + " · " + modelData.pid
                    : modelData.name + (modelData.pids.length > 1 ? " · " + modelData.pids.length + " instances" : "")
                  elide: Text.ElideRight
                ...
                Button {
                  ...
                  verticalPadding: Style.space(2)
                  bordered: true
                  active: modelData.route === "Ethernet"
                  onClicked: root.moveProcesses(modelData.pids || [modelData.pid], "Ethernet")
                }
                Button {
                  width: parent.width * 0.24
                  text: "Wi-Fi"
                  ...
                  verticalPadding: Style.space(2)
                  bordered: true
                  active: modelData.route === "Wi-Fi"
                  onClicked: root.moveProcesses(modelData.pids || [modelData.pid], "Wi-Fi")
                }
              }
            }
            ScrollBar.vertical: ScrollBar { policy: ScrollBar.AsNeeded }
  ...
  // One Internet route mode pill.
  component RouteModePill: Button {
    id: pill
    ...
    active: root.internetRouteMode === mode
    hasCursor: root.cursorActive && root.focusSection === "route" && root.routeIndex === index

    onHovered: function(isHovered) {
      if (!isHovered) return
      root.cursorActive = true
      root.focusSection = "route"
      root.routeIndex = pill.index
    }
  }
